...
class RiderLabel extends Component {
  ...
  render() {
    ...
    const { distanceCovered, rideDurationInSeconds, powerOutput, weight, speedInMillimetersPerHour, heartRateInBpm } = position;
    ...
    return <g className="rider-label" transform={`translate(${(50000 - width / 2) / scale},${10000 / scale})`}>
        ...
        {this.renderData(useMetric ? 'KM' : 'MILES', Math.round(distanceCovered / distFactor / 100) / 10, 20000)}
        ...
        { heartrate ? this.renderData('H/R', heartRateInBpm, 240000) : undefined }
    </g>
  }
